...
export default function ServicesPage() {
  ...
  return (
    <PageLayout>
      {/* Main container to provide clean vertical spacing between sections */}
      <div className="space-y-12 py-6">
        ...
        {/* Services Grid & Loading States */}
        <section className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {loading ? (
            Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                {/* Fixed: Moved sizing entirely into the className prop */}
                <Skeleton className="h-10 w-10 rounded-2xl" />
                <Skeleton className="mt-6 h-5 w-3/4 rounded-lg" />
                <Skeleton className="mt-4 h-4 w-full rounded-lg" />
                <Skeleton className="mt-2 h-4 w-2/3 rounded-lg" />
              </div>
            ))
          ) : error ? (
            <div className="col-span-full rounded-[2rem] border border-red-200 bg-red-50 p-10 text-center font-semibold text-red-700">
              {error}
            </div>
