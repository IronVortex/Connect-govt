...
> = {
  VERIFIED: { label: 'VERIFIED', icon: CheckCircle2, bg: 'bg-emerald-50', text: 'text-emerald-700', dot: 'bg-emerald-500' },
  REVIEW_REQUIRED: { label: 'REVIEW_REQUIRED', icon: AlertCircle, bg: 'bg-amber-50', text: 'text-amber-700', dot: 'bg-amber-500' },
  REJECTED: { label: 'REJECTED', icon: XCircle, bg: 'bg-red-50', text: 'text-red-700', dot: 'bg-red-500' },
  MATCHED: { label: 'VERIFIED', icon: CheckCircle2, bg: 'bg-emerald-50', text: 'text-emerald-700', dot: 'bg-emerald-500' },
  DETECTED: { label: 'VERIFIED', icon: CheckCircle2, bg: 'bg-emerald-50', text: 'text-emerald-700', dot: 'bg-emerald-500' },
  NEEDS_REVIEW: { label: 'REVIEW_REQUIRED', icon: AlertCircle, bg: 'bg-amber-50', text: 'text-amber-700', dot: 'bg-amber-500' },
  MISMATCHED: { label: 'REJECTED', icon: XCircle, bg: 'bg-red-50', text: 'text-red-700', dot: 'bg-red-500' },
  UNKNOWN: { label: 'UNKNOWN', icon: HelpCircle, bg: 'bg-slate-100', text: 'text-slate-600', dot: 'bg-slate-400' },
  PENDING: { label: 'PENDING', icon: Clock, bg: 'bg-slate-100', text: 'text-slate-500', dot: 'bg-slate-300' },
};
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${meta.bg} ${meta.text}`}>
...
async function downloadFile(uploadId: string, filename: string) {
  try {
    const res = await apiClient.get(`/upload/files/${uploadId}`, { responseType: 'blob' });
...
export default function WalletPage() {
  ...
  const statItems = [
    { label: 'Verified docs', value: walletDocs.length, icon: ShieldCheck, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Total uploads', value: allUploads.length, icon: Layers, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Under review', value: allUploads.filter((d) => d.detectionStatus === 'REVIEW_REQUIRED').length, icon: AlertCircle, color: 'text-amber-600', bg: 'bg-amber-50' },
    { label: 'Expiring soon', value: walletDocs.filter((d) => { const days = expiryDaysLeft(d); return days !== null && days <= 14; }).length, icon: CalendarClock, color: 'text-rose-600', bg: 'bg-rose-50' },
  ];
          ...
          {(['wallet', 'history'] as const).map((t) => (
            <button
              ...
              className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${
                tab === t ? 'bg-white text-[#1D61FF] shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              {t === 'wallet' ? (
