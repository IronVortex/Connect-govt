...
export class UploadsService {
  ...
  async detectDocumentType(filename: string, content?: string): Promise<string> {
    const lower = filename.toLowerCase();
    if (lower.includes('passport')) return 'Passport';
    if (lower.includes('id') || lower.includes('identity')) return 'ID Card';
    ...
    if (lower.includes('license')) return 'Driving License';
    return 'Unknown';
  }
}
